...
const PersonList = (props: {
  ...
  >(
    useSelector((state: ItemProps[]) => state).map((item) => ({
      name: item.name,
      price: item.price,
      people: []
    }))
  ...
  const onClickPeopleButton = (itemIndex: number, peopleIndex: number) => {
    setItemList((prev) => {
      ...
      if (currentPeopleList.includes(peopleIndex)) {
        itemListToBe[itemIndex].people = currentPeopleList.filter(
          (num) => num != peopleIndex
        );
      } else {
  ...
  const onDeletePersonButton = (itemIndex: number, peopleIndex: number) => {
    const remainingSavedPeopleIndex = itemList[itemIndex].people.filter(
      (num) => num != peopleIndex
    );
    const remainingSavedPeople = people.filter((_, idx) =>
      remainingSavedPeopleIndex.includes(idx)
    );
    const newPeople = people.filter((_, num) => num != peopleIndex);
    setPeople(newPeople);
    setItemList((prev) => {
      const itemListToBe = [...prev];
      const peopleListToBe = newPeople.map((ppl, idx) => ({
        name: ppl,
        index: idx
      }));
      itemListToBe[itemIndex].people = peopleListToBe
        .filter((ppl) => remainingSavedPeople.includes(ppl.name))
        .map((ppl) => ppl.index);
      return itemListToBe;
    });
  };
  ...
  const startSplitBill = () => {
    ...
    splitBillPerson.forEach((ppl, idx) => {
    ...
    <>
                ...
                {people.map((ppl, jdx) => (
                  <div key={jdx} className="col-auto py-3">
                    <div className="btn-group">
                      <button
                        type="button"
                        onClick={() => onClickPeopleButton(idx, jdx)}
                        className={`btn ${item.people.includes(jdx) ? "btn-success" : "btn-outline-secondary"} position-relative`}
                      >
                        {ppl}
                      </button>
                      <button
                        onClick={() => onDeletePersonButton(idx, jdx)}
      ...
      <div className="position-sticky bottom-0 left-0 py-3 bg-white">
        <div className="row justify-content-between">
          <div className="col-12 mb-3">Input People Name</div>
          ...
          <div className="col-12">
            <button
              disabled={itemVoidLength > 0}
              className={`btn ${itemVoidLength > 0 ? "btn-secondary" : "btn-success"} w-100`}
              type="button"
